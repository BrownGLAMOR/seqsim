...
public class FullCondSCPolySmoothed {
	public static void main(String[] args) throws IOException {
		...
		// agent preferences		
		double[] ORDER = new double[] {2};		// order of polynomial agent
		int preference = 2;							// MDP agent preference
		...
		int no_per_iteration = 300000/no_agents;			// no. of games played in each Wellman iteration		
		...
		boolean smooth_prices = false;
		double kappa = 0.5;								// target learning parameter
		double weight = kappa/(1-kappa);				// corresponding smoothing parameter in code 
		...
		for (int o = 0; o < ORDER.length; o++) {
			...
			System.out.println("Generating initial PP for order = " + order);
			...
			for (int i = 0; i<no_agents; i++)
				poly_agents[i] = new PolynomialAgent(new MenezesValue(max_value, rng, decreasing), no_agents, order);
	...
	//			if (decreasing == true)
	//				poly_agents[0].valuation.delta_x = v[i] + v[i]*v[i];
	//			else
	//				poly_agents[0].valuation.delta_x = v[i] + java.lang.Math.sqrt(v[i]);
			...
			for (int it = 0; it < no_iterations; it++) {
				...
				if (smooth_prices == true)
					PP[it+1].addJcde(PP[it], weight);
			...
			if (print_strategy == true){
				...
				FileWriter fw_strat;
				if (smooth_prices == true)
					fw_strat = new FileWriter("/Users/jl52/Desktop/Amy_paper/workspace/paper/updates/smoothedpoly" + order + "_" + no_agents + "_" + precision + "_" + no_iterations + "_" + kappa + ".csv");
				else
					fw_strat = new FileWriter("/Users/jl52/Desktop/Amy_paper/workspace/paper/updates/poly" + order + "_" + no_agents + "_" + precision + "_" + no_iterations + ".csv");
