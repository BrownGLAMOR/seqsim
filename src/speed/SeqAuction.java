...
public class SeqAuction {
	...
	// play the auction but with probability eta, make agent[0] bid randomly w/ prob eta and other agents bid randomly w/ prob eta2 
	public void playWithNoise(double max_price, double eta, double eta2, Random rng) throws IOException {
		// reset our internal variables
...
//			a.reset(this);
		...
		for (int i = 0; i<no_goods; i++) {
			...
			for (int j = 0; j<agents.length; j++) {
				...
				// Randomize first agent's bid with certain probability
				double bid;
				if ((j == 0 && rng.nextDouble() < eta) || (j != 0 && rng.nextDouble() < eta2)){
//					System.out.println("overwrites...");
					bid = rng.nextDouble()*max_price;
				}else
					bid = agents[j].getBid(i);
	...
	// Testing: playWithNoise. Just see if it works. 
	public static void main(String args[]) throws IOException {
		
		int no_goods = 2, no_agents = 3, nth_price = 2;
		double max_price = 1.0, precision = 0.05;
		...
		for (int i = 0; i<no_agents; i++){
			poly_values[i] = new MenezesMultiroundValue(max_price, rng, decreasing);
