...
public class FullMDPNumGoodsSeqAgent extends SeqAgent {
	...
	public FullMDPNumGoodsSeqAgent(JointDistributionEmpirical jde, Value valuation, int agent_idx) {
	...
	public void computeFullMDP(JointDistribution pd){	
		...
		genPrices = Cache.getCartesianProduct(prices, t);
		...
		for (int x = 0; x<=jde.no_goods; x++) {
			for (double[] realized : genPrices)
			{
				...
				V[x][t].put(new DoubleArray(realized), valuation.getValue(x)); 
			}
		}
		...
		for (t = jde.no_goods-1; t>-1; t--){ 
			...
			genPrices = Cache.getCartesianProduct(prices, t);	// possible realized historical prices		
...
//			    		System.out.println();
	...
	public void reset(SeqAuction auction) {
		...
		no_goods_won = 0;
		// TODO: to get reset semantics right, I think we need to compute MDP here, as opposed to
		// in the constructor?
	}
