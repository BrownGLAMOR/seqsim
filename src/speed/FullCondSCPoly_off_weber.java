...
// Do self-confirming UNIFORM OFF-PLOICY updates initiated from Polynomial strategies, in Weber's set up
public class FullCondSCPoly_off_weber {
	public static void main(String[] args) throws IOException {
		...
		// agent preferences		
		double[] ORDER = new double[] {1, 2, 10};
		...
		int no_per_iteration = 50000;			// no. of games played in each Wellman iteration		
		...
		for (int o = 0; o < ORDER.length; o++) {
			...
			UnitValue[] poly_values = new UnitValue[no_agents];
			for (int i = 0; i<no_agents; i++){
				poly_values[i] = new UnitValue(max_value, rng);
			...
			for (int it = 0; it < no_iterations; it++) {
				...
				for (int i = 0; i < no_agents; i++) {
					mdp_agents[i] = new FullCondMDPAgent4(poly_values[i], i, preference, epsilon, discretize_value, v_precision);
			...
			if (print_strategy == true){
				FileWriter fw_strat = new FileWriter("/Users/jl52/Desktop/Amy_paper/workspace/paper/june1st/uniform/weberpoly" + order + "_" + no_agents + "_" + precision + "_" + no_iterations + ".csv");
			...
			if (compute_epsilon == true) {
				...
				FileWriter fw_EUdiff = new FileWriter("/Users/jl52/Desktop/Amy_paper/workspace/paper/june1st/uniform/weberpoly" + order + "_EUdiff_" + no_agents + "_" + precision + "_" + no_iterations + ".csv");
