 ...
 * Clients connect to the server through Sequentian2ndPriceAuctionServer objects,
 * which communicate with the clients and update the state of the auction based 
 * on actions taken by the competing agents trying to win the auctions.
 * 
 * The auction itself is instantiated here, but most of the mechanics of running
 * the auction are handled in the Simultaneous2ndPriceAuctionServerThread
 * class.
 * 
...
public class Sequential2ndPriceAuctionServer {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
    	...
    	Vector<Sequential2ndPriceAuctionServerThread> myThreads = new Vector<Sequential2ndPriceAuctionServerThread>();
      ...
      //Try reading in the IP and socket number from the text file...
        ...
        while(!started){ //(listening && !myAuction.isOver() ){
        	...
        	Sequential2ndPriceAuctionServerThread aThread;
        	aThread= new Sequential2ndPriceAuctionServerThread( serverSocket.accept(), myAuction, numPlayersNeeded);
