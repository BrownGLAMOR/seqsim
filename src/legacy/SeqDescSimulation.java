...
public class SeqDescSimulation {

	private List<Agent> agents;
	private List<SBAuction> auctions;
	
	public SeqDescSimulation(List<Agent> agents, List<SBAuction> auctions) {
		this.agents = agents;
			...
			do {
				...
				for (int i = 0; i<agents.size(); i++) {
					...
					if (i_bids.containsKey(x))
						auctions.get(x).submitBid(i, i_bids.get(x)); // bids below ask will get rejected.
				}
				...
				rounds++;
			} while (auctions.get(x).getNoWinners() == 0 && auctions.get(x).getLastAskPrice() > auctions.get(x).getReservePrice());
